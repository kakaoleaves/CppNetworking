...
int main(int argc, char* argv[]) // three arguments to be checked later
{
	...
	while (true)
	{
		...
		int sendBytes = ::sendto(clientSocket, sendBuffer, (int)strlen(sendBuffer), 0, (SOCKADDR*)&serverAddr, sizeof(serverAddr));
		...
		int recvBytes = ::recvfrom(clientSocket, recvBuffer, sizeof(recvBuffer), 0, (SOCKADDR*)&recvAddr, &recvAddrSize);
